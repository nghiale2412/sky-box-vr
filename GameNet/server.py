    ...
    try:
      ...
      if command[0] == 'request_scores':
          ...
          if (len(PLAYER_SCORE) == 2):
              for k, v in PLAYER_SCORE.iteritems():
                  if v == 5:
                      data = "speed+,10"
                      sock.sendto(data,CONNECTED_PLAYERS["Player1"])
                      sock.sendto(data,CONNECTED_PLAYERS["Player2"])
                      print "Speed increased first time"
                      print data
                  if v == 10:
                      data = "speed++,20"
                      sock.sendto(data,CONNECTED_PLAYERS["Player1"])
                      sock.sendto(data,CONNECTED_PLAYERS["Player2"])
                      print "Speed increased second time"
                      print data
                  if v == 20:
                      data = "winner," + k + "," +  str(int(PLAYER_SCORE["Player1"])) + "," + str(int(PLAYER_SCORE["Player2"]))
                      sock.sendto(data,CONNECTED_PLAYERS["Player1"])
                      sock.sendto(data,CONNECTED_PLAYERS["Player2"])
